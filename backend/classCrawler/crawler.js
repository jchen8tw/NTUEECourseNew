...
const collectAllData = new Promise((resolve, reject) => {
  ...
  const WEEK_MAX = 5;
  const PAGE_MAX = 5;
  for (let building = 9; building <= 9; building++) {
    for (let week = 1; week <= WEEK_MAX; week++) {
      for (let page = 1; page <= PAGE_MAX; page++) {
        const mainUrl = `http://gra206.aca.ntu.edu.tw/classrm/index.php/acarm/webcr-use1-new?Type=1&page=${page}&SYearDDL=1081&BuildingDDL=${building}&Week=${week}&Capacity=1&SelectButton=%E5%88%AA%E9%99%A4`;
        fetchClassDataURL(mainUrl)
          .then(data => {
            ...
            counter++;
            if (counter === WEEK_MAX * PAGE_MAX) resolve(allData);
          })
          .catch(error => {
            return reject(error);
          });
  ...
  try {
    ...
    csvWriter
      ...
      .then(() => {
        ...
        console.log(data.filter(item => item[2].includes('固態電子')));
      });
  } catch (err) {
    console.error(err);
  }
